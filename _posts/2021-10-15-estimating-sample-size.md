...
For any of these, [G\*Power](https://www.psychologie.hhu.de/arbeitsgruppen/allgemeine-psychologie-und-arbeitspsychologie/gpower) remains the standard free tool — pick the matching test family and design under "Test family → t tests / F tests" and plug in the effect size from the table above; for the mixed design specifically, use G\*Power's dedicated "ANOVA: Repeated measures, within-between interaction" test. If you prefer to stay in R, the [`pwr` package](https://cran.r-project.org/web/packages/pwr/index.html) covers the t-tests and the one-way ANOVA (`pwr.t.test()`, `pwr.anova.test()`), but doesn't have a repeated-measures/mixed-design function — for that, use `wp.rmanova()` from the [`WebPower`](https://webpower.psychstat.org/wiki/) R package (or its free [online calculators](https://webpower.psychstat.org/wiki/) if you'd rather not install anything).
